...
void systems::loading_actual_image(
    ...
    auto [entity, image_loading] = query.single().value();

    int width, height, channels;
    stbi_set_flip_vertically_on_load(true);
    uint8_t* pixels = stbi_load(
        image_loading.path.c_str(), &width, &height, &channels, STBI_rgb_alpha
    );
    if (!pixels) {
        sprite_logger->error(
            "Failed to load image at path: {}. This may lead to sprite being "
            "ignored or using other images if an new image was loaded to the "
    ...
    cmd.entity(entity).emplace(image, image_view);
}
