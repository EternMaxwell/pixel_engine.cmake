    ...
    command.spawn(
        PipelineBundle{
            ...
            .attribs{
                .attribs{
                    VertexAttrib{
                        0, 3, GL_FLOAT, false, sizeof(TextVertex),
                        offsetof(TextVertex, position)},
                    VertexAttrib{
                        1, 4, GL_FLOAT, false, sizeof(TextVertex),
                        offsetof(TextVertex, color)},
                    VertexAttrib{
                        2, 2, GL_FLOAT, false, sizeof(TextVertex),
...
void pixel_engine::font_gl::systems::draw(
    ...
    Query<
        ...
        window_query,
    Query<
        Get<Pipeline, Buffers, Images>, With<TextPipeline, ProgramLinked>,
        Without<>>
        pipeline_query) {
            ...
            for (auto [window_size] : window_query.iter()) {
                for (auto [text, transform] : text_query.iter()) {
                    int err = FT_Set_Char_Size(
                        text.font_face, 0, text.pixels, 1920, 1080);
                    if (err) {
                        continue;
                    }
                    ...
                    glGetTextureLevelParameteriv(
                        images.images[0].texture, 0, GL_TEXTURE_WIDTH,
                        &image_width);
                    glGetTextureLevelParameteriv(
                        images.images[0].texture, 0, GL_TEXTURE_HEIGHT,
                        &image_height);
                    ...
                    for (auto c : text.text) {
                        ...
                        int err = FT_Load_Glyph(
                            text.font_face, glygh_index, FT_LOAD_DEFAULT);
                        if (err) {
                            continue;
                        }
                        err = FT_Render_Glyph(
                        ...
                        if (full_height > image_height ||
                            ...
                            glCopyImageSubData(
                                images.images[0].texture, GL_TEXTURE_2D, 0, 0,
                                0, 0, new_texture, GL_TEXTURE_2D, 0, 0, 0, 0,
                                image_width, image_height, 1);
                            glDeleteTextures(1, &images.images[0].texture);
                            images.images[0].texture = new_texture;
                        }
                        glTextureSubImage2D(
                            images.images[0].texture, 0, x, y - bitmap.rows,
                            bitmap.width, bitmap.rows, GL_RED, GL_FLOAT,
                            buffer);
                        delete buffer;
                    ...
                    float height = text.size
                                       ? text.size * lines
                                       : (ortho_proj.top - ortho_proj.bottom) *
                                             text.pixels * lines /
                                             window_size.height;
                    float s1 = 0;
                    float t1 = 1;
                    ...
                    glNamedBufferData(
                        pipeline.vertex_buffer.buffer, sizeof(vertices),
