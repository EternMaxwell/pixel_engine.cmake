...
PixelRenderer::Context::Context(
    Device& device, Swapchain& swapchain, Queue& queue
)
    : device(&device), swapchain(&swapchain), queue(&queue) {}
...
void PixelRenderer::begin(
    ...
    context   = Context{device, swapchain, queue};
...
void PixelRenderer::draw(const PixelBlock& block, const BlockPos2d& pos2d) {
    if (!context.has_value()) return;
    auto& ctx = context.value();
...
void PixelRenderer::set_block_data(
...
void PixelRenderer::flush() {
    ...
    if (ctx.block_model_offset) {
        ...
        render_pass_info.setRenderArea({{0, 0}, ctx.swapchain->extent});
        ...
        command_buffer->setScissor(
            0, vk::Rect2D({0, 0}, ctx.swapchain->extent)
        );
        command_buffer->setViewport(
            0, vk::Viewport(
                   0.0f, 0.0f, ctx.swapchain->extent.width,
                   ctx.swapchain->extent.height, 0.0f, 1.0f
               )
        );
