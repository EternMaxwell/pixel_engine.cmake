    ...
    namespace entity {
        ...
        class Query<std::tuple<Qus...>, std::tuple<Exs...>> {
           private:
            ...
            decltype(registry.view<Qus...>(entt::exclude_t<Exs...>{}).each()) m_iter;

           public:
            Query(entt::registry& registry) : registry(registry) {
                m_iter = registry.view<Qus...>(entt::exclude_t<Exs...>{}).each();
            }
             ...
             */
            auto iter() { return iterator(registry.view<Qus...>(entt::exclude_t<Exs...>{}).each()); }
        ...
        class Query<std::tuple<entt::entity, Qus...>, std::tuple<Exs...>> {
           ...
           public:
             ...
             * @return The iterator for the query.
             */
            auto iter() { return registry.view<Qus...>(entt::exclude_t<Exs...>{}).each(); }
             ...
             */
            auto single() {
                auto start = *(registry.view<Qus...>(entt::exclude_t<Exs...>{}).each().begin());
                if (registry.view<Qus...>(entt::exclude_t<Exs...>{}).each().begin() !=
        ...
        template <typename Qu1, typename Qu2, typename... Qus1, typename... Qus2, typename... Exs1, typename... Exs2>
        struct queries_contrary<Query<std::tuple<Qu1, Qus1...>, std::tuple<Exs1...>>,
                ...
                bool value() {
                    if constexpr (sizeof...(Args1) == 0) {
                        ...
                        if constexpr (sizeof...(Args2) > 0) {
                            return first_tuple_contains_any_type_in_second_tuple<std::tuple<Args1...>,
                                                                                 std::tuple<Args2...>>{}
                                .value();
                        } else {
                            return false;
            ...
            struct tuples_has_no_same_non_const_type<std::tuple<Args1...>, std::tuple<Args2First, Args2...>> {
                bool value() {
                    if constexpr (sizeof...(Args1) == 0) {
                        ...
                        if constexpr (sizeof...(Args2) > 0) {
                            return tuples_has_no_same_non_const_type<std::tuple<Args1...>, std::tuple<Args2...>>{}
                                .value();
                        } else {
                            return true;
                        }
            ...
            bool value() {
                if constexpr ((!std::is_same_v<Qu1, entt::entity>)&&(!std::is_same_v<Qu2, entt::entity>)) {
                    ...
                    // false
                    if (tuples_has_no_same_non_const_type<std::tuple<Qu1, Qus1...>, std::tuple<Qu2, Qus2...>>{}
                            .value()) {
                        return false;
                    }
                    ...
                    // if Exs1 contains any non-const Type in Qus2 return
                    // false
                    if (first_tuple_contains_any_type_in_second_tuple<std::tuple<Exs2...>, std::tuple<Qus1...>>{}
                            .value()) {
                        return false;
                    }
                    ...
                    // if Qus1 and Qus2 has no same non-const type return
                    // false
                    if (tuples_has_no_same_non_const_type<std::tuple<Qus1...>, std::tuple<Qu2, Qus2...>>{}.value()) {
                        return false;
                    }
                    ...
                    // if Exs2 contains any non-const Type in Qus1 return
                    // false
                    if (first_tuple_contains_any_type_in_second_tuple<std::tuple<Exs1...>, std::tuple<Qus2...>>{}
                            .value()) {
                        return false;
                    }
                    ...
                    // if Qus1 and Qus2 has no same non-const type return
                    // false
                    if (tuples_has_no_same_non_const_type<std::tuple<Qu1, Qus1...>, std::tuple<Qus2...>>{}.value()) {
                        return false;
                    }
                }
